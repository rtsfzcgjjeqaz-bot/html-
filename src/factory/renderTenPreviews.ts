...
import { renderPreviewFromStructure } from "./sequentialRenderer";
...
export function renderTenPreviews(runId: string) {
  ...
  for (const file of files) {
    ...
    if (!qa.passed) throw new Error(`qualityGate blocked preview render: ${qaPath}`);
    renderPreviewFromStructure(structurePath, path.join(previewDir, file.replace(".structure.json", ".preview.mp4")));
  }
}

if (require.main === module) {
  const runId = arg("--runId");
  if (!runId) throw new Error('Usage: npm run factory:render10:preview -- --runId "run_..."');
  renderTenPreviews(runId);
}
